...
public class OffMeetAdapter extends BaseQuickAdapter<InterfaceMeet.pbui_Item_MeetMeetInfo, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, InterfaceMeet.pbui_Item_MeetMeetInfo item) {
        ...
        switch (item.getStatus()) {
            case 0:
                state = mContext.getResources().getString(R.string.Uninitiated_meeting);
                break;
            case 1:
                state = mContext.getResources().getString(R.string.Already_in_session);
                break;
            case 2:
                state = mContext.getResources().getString(R.string.is_Closed_meeting);
                break;
            default:
                state = mContext.getResources().getString(R.string.default_str);
                break;
        }
        helper.setText(R.id.item_tv_id, String.valueOf(item.getId()))
                ...
                .setText(R.id.item_tv_secrecy, item.getSecrecy() == 1 ? mContext.getResources().getString(R.string.secrecy) : "")
        ...
        int color = (selectMeetId == item.getId()) ? mContext.getResources().getColor(R.color.btn_fill_color)
    ...
    public int getSelectPosion() {
        for (int i = 0; i < mData.size(); i++) {
            if (mData.get(i).getId() == selectMeetId) {
                return i;
            }
        }
        return -1;
    ...
    public InterfaceMeet.pbui_Item_MeetMeetInfo getSelectedMeet() {
        for (int i = 0; i < mData.size(); i++) {
            InterfaceMeet.pbui_Item_MeetMeetInfo meetInfo = mData.get(i);
    ...
    public void updateSelected() {
        for (int i = 0; i < mData.size(); i++) {
            if (selectMeetId == mData.get(i).getId()) {
                notifyDataSetChanged();
                return;
            }
        }
