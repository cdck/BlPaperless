...
public class BoardAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void notifyChecks() {
        ...
        for (int i = 0; i < mData.size(); i++) {
            if (checks.contains(mData.get(i).getMemberInfos().getPersonid())) {
                ids.add(mData.get(i).getMemberInfos().getPersonid());
            }
        }
        checks = ids;
    ...
    public void setAllCheck(boolean all) {
        ...
        if (all) {
            for (int i = 0; i < mData.size(); i++) {
                checks.add(mData.get(i).getMemberInfos().getPersonid());
            }
        }
        notifyDataSetChanged();
    }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        ((ViewHolder) holder).play_btn.setSelected(checks.contains(mData.get(position).getMemberInfos().getPersonid()));
