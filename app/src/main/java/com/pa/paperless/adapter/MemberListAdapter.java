...
public class MemberListAdapter extends BaseAdapter {
    ...
    public void notifyChecks() {
        ...
        for (int i = 0; i < datas.size(); i++) {
            if (checks.contains(datas.get(i).getMemberInfos().getPersonid())) {
                ids.add(datas.get(i).getMemberInfos().getPersonid());
            }
        }
        checks = ids;
    ...
    public List<String> getCheckedName() {
        ...
        for (int i = 0; i < datas.size(); i++) {
            //获取选中的人员的名字
            if (checks.contains(datas.get(i).getMemberInfos().getPersonid())) {
                String name = datas.get(i).getMemberInfos().getName().toStringUtf8();
                checkName.add(name);
                if (checkName.size() > 3) {
    ...
    public boolean setAllChecked() {
        ...
        if (checks.size() == datas.size()) {
            ...
            checks.clear();
            for (int i = 0; i < datas.size(); i++) {
                checks.add(datas.get(i).getMemberInfos().getPersonid());
            }
            isall = true;
        }
        notifyDataSetChanged();
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        holder.member_tv.setText(memberInfo.getMemberInfos().getName().toStringUtf8());
        //设置names集合中的值
        holder.member_tv.setSelected(checks.contains(memberInfo.getMemberInfos().getPersonid()));
        return view;
    }
