...
//                LogUtil.v(TAG, "Get H264 Buffer Success! flag = " + bufferInfo.flags + ", pts = " + bufferInfo.presentationTimeUs + "");
                ...
                if (isDebug) {
                    ...
                    if (outputBuffer != null) {
                        ...
                        Log.v(TAG, "写入文件中...");
                        mediaMuxer.writeSampleData(m_videoTrackIndex, outputBuffer, bufferInfo);
                    }
                }
...
//                    read2File(keyframe);
//                    allCount++;
//                    LogUtil.i(TAG, "发送方 发送关键帧数据 关键帧之间的个数=" + count + ", pts= " + bufferInfo.presentationTimeUs + ", size= " + keyframe.length + ", 发送数据总量：" + allCount);
//                    count = 1;
